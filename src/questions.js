...
async function load(){
  const [q,a,v]=await Promise.all([
    client.from('colleague_questions').select('id,author_id,title,body,category,school_level,topic_tag,created_at').eq('moderation_status','published').order('created_at',{ascending:false}).limit(100),
...
function render(){
  ...
  $('#questions').innerHTML=filtered.length?filtered.map(q=>{
    ...
    return `<article class="question-card" id="soru-${esc(q.id)}"><div class="document-card-meta"><span>${esc(q.category)}${q.school_level?' · '+esc(q.school_level):''}${q.topic_tag?' · '+esc(q.topic_tag):''}</span><span>${date(q.created_at)}</span></div><h2>${esc(q.title)}</h2><p class="question-copy">${esc(q.body)}</p><p class="question-author">Soran: ${author(q.author_id)} · ${list.length} cevap <button type="button" class="report-action" data-report-type="question" data-report-id="${esc(q.id)}">Bildir</button></p><details class="thread"><summary>Yanıtlar <span>${list.length}</span></summary><div class="question-answers">${list.map(a=>{const count=counts.get(a.id)||0,selected=votes.some(v=>v.answer_id===a.id&&v.voter_id===user?.id);return `<div class="answer-card"><p>${esc(a.body)}</p><div class="answer-footer"><span>${author(a.author_id)} · ${date(a.created_at)}</span><button type="button" data-vote="${esc(a.id)}" aria-pressed="${selected}" ${!user||a.author_id===user.id?'disabled':''}>${selected?'✓ ':''}Faydalı buldum · ${count}</button><button type="button" class="report-action" data-report-type="answer" data-report-id="${esc(a.id)}">Bildir</button></div></div>`}).join('')}</div></details>${user?`<details class="reply-composer"><summary>Yanıt yaz ↗</summary><form class="answer-form" data-answer="${esc(q.id)}"><label>Cevap yaz<textarea name="body" minlength="10" maxlength="4000" required placeholder="Deneyimini ve gerekçeni paylaş. Öğrenci adı veya tanınabilir bilgi yazma."></textarea></label><button type="submit">Cevapla ↗</button></form></details>`:'<a href="hesap.html">Cevaplamak için giriş yap ↗</a>'}</article>`
  }).join(''):'<p>Bu aramaya uygun soru bulunamadı.</p>';if(location.hash.startsWith('#soru-'))requestAnimationFrame(()=>document.getElementById(decodeURIComponent(location.hash.slice(1)))?.scrollIntoView({block:'start'}));
}
