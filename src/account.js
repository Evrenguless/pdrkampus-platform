...
async function refreshUser(){const {data:{session},error:sessionError}=await client.auth.getSession();if(sessionError)throw sessionError;if(recovering)return;if(!session){$('#accountFormPanel').hidden=false;$('#accountProfile').hidden=true;setMessage('');return}const {data:{user},error}=await client.auth.getUser();if(error)throw error;const {data:profile}=await client.from('profiles').select('display_name').eq('id',user.id).maybeSingle();showUser(user,profile)}
